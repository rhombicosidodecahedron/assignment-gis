<?php
    //return selected area of traffic light
    ...
    $query .= ")";
    $query .= "SELECT jsonb_build_object('type','FeatureCollection','features', jsonb_agg(features.feature))FROM (SELECT jsonb_build_object('type','Feature','geometry', ST_AsGeoJSON(polygon)) AS feature FROM (SELECT ST_AsText(geom) as polygon FROM t LIMIT (SELECT COUNT(ogc_fid) FROM traffic_signals_in_chapel_hill ) ) inputs) features;";
